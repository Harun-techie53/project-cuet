...
const ProfileSection = () => {
    ...
    const fetchUserPhoto = async (photo) => {
        try {
            const res = await api.get(`/auth/fetchUserPhoto/${photo}`, config);

            res.data && setImagePath(res.data);
        } catch (error) {
            console.log(error);
        }
    }
            ...
            <div className='flex flex-col items-center space-y-4 border-2 bg-gray-200 rounded-md m-5'>
                ...
                <div className='cursor-pointer'>
                    <img 
                        ...
                        src={`../../../../server/public/img/users/${authUserDetails?.photo}`}
                        alt="profile image"
                    />
                </div>
